    ...
    for (int idx = 0; idx < FINGER_MAX; idx++) {
        if (data->fingers[idx].current.x == 0xFFFF || data->fingers[idx].current.y == 0xFFFF) {
            ...
            if (processed->fingers[idx].state == FINGER_DOWN) {
                ...
                if (timer_elapsed32(processed->fingers[idx].t_start) < TAP_TIME_MS && abs(processed->fingers[idx].rel.x) < 100 && abs(processed->fingers[idx].rel.y) < 100) {
    ...
    if (data->finger_cnt == 0) {
        ...
        *gesture_data              = zero;
        ...
        if (tapped) {
            ...
            processed->tap_cnt = 0;
            for (int idx = 0; idx < FINGER_MAX; idx++) {
                if (processed->fingers[idx].tapped && timer_elapsed32(processed->fingers[idx].t_tapped) < TAP_TIME_MS) {
                    processed->tap_cnt++;
                }
        ...
        if (gesture_data->two.gesture_state == GESTURE_NONE) {
            for (int idx = 0; idx < FINGER_MAX; idx++) {
                if ((idx != active_finger_id) && processed->fingers[idx].tapped && (timer_elapsed32(processed->fingers[idx].t_tapped) > TAP_TIME_MS)) {
                    rep_mouse->buttons |= (1 << 0);
                }
            }
        }

        rep_mouse->x                                            = processed->fingers[active_finger_id].frame_move.x * 1.5;
        rep_mouse->y                                            = processed->fingers[active_finger_id].frame_move.y * 1.5;
