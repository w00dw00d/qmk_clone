...
static int16_t latest_tap_time = 0;
...
bool process_iqs5xx(iqs5xx_data_t const* const data, iqs5xx_processed_data_t* processed, report_mouse_t* const rep_mouse, iqs5xx_gesture_data_t* gesture_data) {
    ...
    for (int idx = 0; idx < FINGER_MAX; idx++) {
        if (data->fingers[idx].current.x == 0xFFFF || data->fingers[idx].current.y == 0xFFFF) { // 動いているかの判定
            // finger is up
            if (processed->fingers[idx].state == FINGER_DOWN) {
                processed->fingers[idx].state = FINGER_UP;

                if (idx == 0 && data->finger_cnt == 0) {
                    // tap継続フラグ
                    is_keep_tapping = false;
            ...
            if (!is_keep_tapping && idx == 0) {
                if (timer_elapsed32(latest_tap_time) < DBL_TIME_MS) {
                    // tap継続フラグ
                    is_keep_tapping = true;
                }
            }
    ...
    // if (pointing_device_button != 0 && !is_keep_tapping) {
    //     rep_mouse->buttons |= pointing_device_button;
    // }

    // char *finger_cnt = "";
    // switch (data->finger_cnt) {
    //     case 0:
    //         finger_cnt = "";
    //         break;
    //     case 1:
    ...
    //         break;
    // }
    //if (strcmp(finger_cnt, "") != 0) debug_string(strcat("finger_cnt:", finger_cnt));
        ...
        *gesture_data              = zero;
        ...
        if (tapped) {
            ...
            for (int idx = 0; idx < FINGER_MAX; idx++) {
                if (processed->fingers[idx].tapped &&
                    ...
                    if(idx == 0){
                        one_finger_drag = false;
                        //debug_string("one_finger_drag = false");
                    }
                }
            ...
            //last_tap_time = processed->fingers[0].t_tapped;
        }
        processed->fingers[0].t_tapped = timer_read32();

    } else if (data->finger_cnt == 1) {
        //debug_string("data->finger_cnt == 1");
        ...
        if (gesture_data->two.gesture_state == GESTURE_NONE) {
           ...
           int8_t diff_y = processed->fingers[active_finger_id].last_gesture_point.y - processed->fingers[active_finger_id].last.y;
            ...
            // debug_string("two.gesture_state == GESTURE_NONE)");
            // switch (processed->tap_cnt) {
            //     case 0:
            //         debug_string("processed->tap_cnt 0");
            //         break;
            //     case 1:
            //         debug_string("processed->tap_cnt 1");
            //         break;
            //     case 2:
            //         debug_string("processed->tap_cnt 2");
            //         break;
            //     default:
            //         debug_string("processed->tap_cnt else");
            //         break;
            //     }

            if (is_keep_tapping && (!(rep_mouse->buttons == 1)) &&
                diff_x == 0 &&
                diff_y == 0) {
                rep_mouse->buttons = 1;
            }
                ...
                //         //debug_string("one_finger_drag = true;");
                // }
        }
        rep_mouse->x                                            = processed->fingers[active_finger_id].frame_move.x * 1.5; // / 2;
        rep_mouse->y                                            = processed->fingers[active_finger_id].frame_move.y * 1.5; // / 2;
        ...
        //debug_string("data->finger_cnt == 2");
